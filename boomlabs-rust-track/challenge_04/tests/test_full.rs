...
macro_rules! timeout {
    ($time:expr, $body:block) => {
        ...
        if let Err(any) = receiver
            .recv_timeout(std::time::Duration::from_millis($time))
            .unwrap()
        {
...
fn test_adding_rooms_2() {
    ...
    dungeon
        .set_link("Entrance", Direction::East, "Hallway 1")
        .unwrap();
    dungeon
        .set_link("Hallway 1", Direction::East, "Side Closet")
        .unwrap();
    dungeon
        .set_link("Hallway 1", Direction::North, "Hallway 2")
        .unwrap();
    dungeon
        .set_link("Hallway 2", Direction::South, "Side Closet")
        .unwrap();
    dungeon
        .set_link("Hallway 2", Direction::West, "Treasure Room")
        .unwrap();
    dungeon
        .set_link("Side Closet", Direction::South, "Entrance")
        .unwrap();
    ...
    assert_eq!(
        dungeon.get_room("Treasure Room").unwrap().name,
        "Treasure Room"
    );
    ...
    assert_eq!(
        dungeon
            .get_next_room("Hallway 1", Direction::West)
            .unwrap()
            .unwrap()
            .name,
        "Entrance"
    ...
    assert_eq!(
        dungeon
            .get_next_room("Hallway 1", Direction::North)
            .unwrap()
            .unwrap()
            .name,
        "Hallway 2"
    ...
    assert_eq!(
        dungeon
            .get_next_room("Hallway 2", Direction::South)
            .unwrap()
            .unwrap()
            .name,
        "Side Closet"
    ...
    assert_eq!(
        dungeon
            .get_next_room("Side Closet", Direction::South)
            .unwrap()
            .unwrap()
            .name,
        "Entrance"
    ...
    assert_eq!(
        dungeon
            .get_next_room("Entrance", Direction::North)
            .unwrap()
            .unwrap()
            .name,
        "Side Closet"
...
fn test_room_links() {
    ...
    dungeon.add_room("Hallway 1").unwrap();
    dungeon
        .set_link("Entrance", Direction::North, "Hallway 1")
        .unwrap();
    assert_eq!(
        all_links(&dungeon, dungeon.get_room("Entrance").unwrap()),
        vec!["Hallway 1"]
    ...
    dungeon.add_room("Hallway 2").unwrap();
    dungeon
        .set_link("Entrance", Direction::South, "Hallway 2")
        .unwrap();
    assert_eq!(
        all_links(&dungeon, dungeon.get_room("Entrance").unwrap()),
    ...
    dungeon
        .set_link("Entrance", Direction::East, "Hallway 3")
        .unwrap();
    assert_eq!(
        all_links(&dungeon, dungeon.get_room("Entrance").unwrap()),
    ...
    dungeon
        .set_link("Entrance", Direction::West, "Hallway 4")
        .unwrap();
    assert_eq!(
        all_links(&dungeon, dungeon.get_room("Entrance").unwrap()),
...
fn test_overwriting_a_room_link() {
    ...
    dungeon.add_room("Hallway 2").unwrap();
    dungeon
        .set_link("Entrance", Direction::North, "Hallway 2")
        .unwrap();
    assert_eq!(
        all_links(&dungeon, dungeon.get_room("Entrance").unwrap()),
        vec!["Hallway 2"]
...
fn test_cyrillic_room_names() {
    ...
    assert!(matches!(
        dungeon.get_room("Кухня"),
        Err(Errors::UnknownRoom(_))
    ));

    assert_eq!(
        dungeon
            .get_next_room("Антре", Direction::North)
            .unwrap()
            .unwrap()
            .name,
        "Хол"
    ...
    assert_eq!(
        dungeon
            .get_next_room("Хол", Direction::South)
            .unwrap()
            .unwrap()
            .name,
        "Антре"
...
fn test_room_errors() {
    ...
    assert!(matches!(
        ...
        Err(Errors::DuplicateRoom(_))
    ));
    assert!(matches!(
        dungeon.get_room("Exit"),
        Err(Errors::UnknownRoom(_))
    ));
}
...
fn test_parsing_no_rooms_or_links() {
    ...
    assert!(matches!(
        dungeon.get_room("Entrance"),
        Err(Errors::UnknownRoom(_))
    ));

    let dungeon = Dungeon::from_reader(TEST_INPUT_3.trim().as_bytes()).unwrap();
    assert_eq!(
        all_links(&dungeon, dungeon.get_room("Pantry").unwrap()),
        Vec::<&str>::new()
    );
}
...
fn test_invalid_parsing() {
    assert!(matches!(
        Dungeon::from_reader("".as_bytes()),
        Err(Errors::LineParseError { line_number: 0 })
    ));
    assert!(matches!(
        Dungeon::from_reader(TEST_INPUT_4.trim().as_bytes()),
        Err(Errors::LineParseError { line_number: 1 })
    ));
    assert!(matches!(
        Dungeon::from_reader(TEST_INPUT_5.trim().as_bytes()),
        Err(Errors::LineParseError { line_number: 3 })
    ));
    assert!(matches!(
        Dungeon::from_reader(TEST_INPUT_6.trim().as_bytes()),
        Err(Errors::UnknownRoom(_))
    ));
    assert!(matches!(
        Dungeon::from_reader(TEST_INPUT_7.trim().as_bytes()),
        Err(Errors::DirectionParseError(_))
    ));
}
...
fn test_parsing_cyrillic_rooms() {
    ...
    assert_eq!(
        dungeon
            .get_next_room("Вход", Direction::West)
            .unwrap()
            .unwrap()
            .name,
        "Хол"
    ...
    assert_eq!(
        dungeon
            .get_next_room("Хол", Direction::East)
            .unwrap()
            .unwrap()
            .name,
        "Вход"
...
fn test_finding_a_direct_path() {
    timeout!(2000, {
        ...
        let path = dungeon
            ...
            .unwrap();
        assert_eq!(
            path.into_iter().map(|p| &p.name).collect::<Vec<_>>(),
...
fn test_finding_a_reflexive_path() {
    timeout!(2000, {
        ...
        let path = dungeon
            .find_path("Treasure Room", "Treasure Room")
            .unwrap()
            .unwrap();
